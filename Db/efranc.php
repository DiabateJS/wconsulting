...
function SaveEfranc($type,$date,$montant,$description)
{
   try{
            $pdo = new PDO(SQL_DSN, SQL_USER, SQL_PASSWORD,array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION ));    

            $q= $pdo->prepare("INSERT INTO e_franc( Type_EF,Date_EF,Montant_EF,Description_EF  ) VALUES(".$pdo->quote($type)." ,".$pdo->quote($date)." , ".$pdo->quote($montant).", ".$pdo->quote($description).")");
            ...
            { 
                ...
                if(isset($id_user))
                {
                  ...
                  $data['Type_EF']= $type;  
                  $data['Date_EF']= $date ; 
                  $data['Montant_EF']= $montant;  
                  $data['Description_EF']= $description ;  
